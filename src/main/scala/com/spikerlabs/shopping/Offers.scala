...
case class CartWithOffer(cart: SomeCart, offer: Offer) extends SomeCart {
  ...
  val total: Int = cart.total - offer(cart.products)
}

object Offers {
  type Offer = (List[Product]) => Int
  ...
  def buyOneGetOneFreeOnApples(products: List[Product]): Int = {
  ...
  def threeForTwoOnOranges(products: List[Product]): Int = {
